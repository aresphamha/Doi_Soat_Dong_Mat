...
def fix_app():
    ...
    old_sql = r'sql_mf01 = f\"\"\"\s*SELECT\s*i\.to_branch_id,\s*i\.code as `Mã chuyển hàng`,\s*i\.double_check_code as `Mã thùng`,\s*l\.barcode as `Mã hàng`,\s*l\.name as `Tên hàng`,\s*l\.unit__name as `ĐVT`,\s*CAST\(IFNULL\(l\.transfer_quantity, 0\) AS DOUBLE\) as `Số lượng chuyển`,\s*CAST\(IFNULL\(l\.store_quantity, 0\) AS DOUBLE\) as `Số lượng nhận`\s*FROM __cdc_kfm_kf_inventories_kf_transfer_items i\s*INNER JOIN __cdc_kfm_ec9d24ab_33bc7bbc_L3___line_items l ON i\._id = l\._root_id\s*WHERE \{shortage_condition\}\s*AND DATE\(DATE_ADD\(i\.transfer_date, INTERVAL 7 HOUR\)\) = \'\{date_str\}\'\s*AND i\.status = 5\s*AND \(l\.barcode NOT LIKE \'CC%\' OR l\.barcode IS NULL\)\s*\"\"\"'
    
    new_sql = '''sql_mf01 = f"""
                    SELECT 
                        i.to_branch_id,
                        i.code as `Mã chuyển hàng`,
                        IFNULL(c.container_codes, i.double_check_code) as `Mã thùng`,
                    ...
                    LEFT JOIN (
                        SELECT _parent_id, GROUP_CONCAT(code SEPARATOR ', ') as container_codes
                        FROM __cdc_kfm_kf_inventories_kf_transfer_items___container_lines
                        GROUP BY _parent_id
                    ) c ON i._id = c._parent_id
    ...
    # Remove the get_ma_thung_from_sheets function
    # It starts with "def get_ma_thung_from_sheets" and ends with "df_shortage = df_mf01..."
    old_func = r'def get_ma_thung_from_sheets\(row_mf01\):.*?df_mf01\[\'Mã thùng\'\] = df_mf01\.apply\(get_ma_thung_from_sheets, axis=1\)'
    content = re.sub(old_func, '', content, flags=re.DOTALL)
