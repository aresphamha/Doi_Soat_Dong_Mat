...
def run_mapping(target_date='2026-07-22'):
    print(f"--- START MEAT FISH RECONCILIATION FOR DATE {target_date} ---")

    
    # 1. Fetch branch mapping (id to name)
    sql_branches = """
    ...
    WHERE i.from_branch_id = '6a34ed56f23028000774139f' -- MF01
      AND DATE(DATE_ADD(i.transfer_date, INTERVAL 7 HOUR)) = '{target_date}'
      AND i.status = 5
    ...
    # 7. Write to Excel
    out_xlsx = f"d:\\Đối soát kho thịt cá\\Doi_Soat_Cheo_{target_date.replace('-', '_')[-5:]}.xlsx"
